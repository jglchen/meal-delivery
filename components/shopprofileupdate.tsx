...
import { UserContextType, ImgDimensionType, ShopDataType} from '@/lib/types';
...
function ShopProfileUpdate({shopData, updateShopData, closeModal}: PropsType){
    ...
    useEffect(() => {
     ...
     async function submitFormData() {
        ...
        try {
           ...
           const { data } = await axios.post('/api/updateshopprofile', formData, { headers: headers });
           setInPost(false);
     ...
     return (userContext && 
        <div className={deliveryStyle.container}>
          <div className={`${deliveryStyle.container_head} ${'text-center'}`}>
             Profile Update for {shopData.shopname}
          ...
          {selectUploaded &&
            ...
            <div>
                <button className="muted-button button" onClick={() => {const pageIdx = pageIndex + 1; setPageIndex(pageIdx); fetchImageData(pageIdx); }}>Load More Images  &raquo;</button>
            </div>
            }
            <div>
                <button className="button" onClick={() => submitCheckedImage()}>Set Checked Image As the Restaurant Profile</button>
            </div>
            </> 
            }
            {imageList.length === 0 &&
