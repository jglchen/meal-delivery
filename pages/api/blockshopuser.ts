...
import {UserJwtPayload, UserRecord, ShopBlockUsers, NoAuthorization, ProcessResult} from '@/lib/types';
const APP_SECRET = process.env.NEXT_PUBLIC_JWT_APP_SECRET as string;
import type { NextApiRequest, NextApiResponse } from 'next';

export default async function handler(req: NextApiRequest, res: NextApiResponse<ShopBlockUsers | NoAuthorization | ProcessResult>) {
    try {
        if (req.method === 'PUT'){
            //Check Authorization
            ...
            const resp = await db.collection('restaurants').doc(shopId).update({blockusers: firestore.FieldValue.arrayUnion(userid)});
            res.status(200).json({shop_blockusers: 1, shopId, userId: userid});   
