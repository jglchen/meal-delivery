...
import {UserJwtPayload, UserRecord, MealDataType, NoAuthorization, ProcessResult} from '@/lib/types';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<MealDataType | NoAuthorization | ProcessResult>) {
    try {
        if (req.method === 'POST'){
            //Check Authorization
            ...
            let shopId = '';     
            const postedData: {mealname?: string, mealdescr?: string, unitprice?: number} = {};
            if (fields?.length){
                for (const field of fields) {
                    if (field[0] === 'mealname' || field[0] === 'mealdescr'){
                       postedData[field[0]] = field[1];
                    }else if (field[0] === 'unitprice'){
                        postedData[field[0]] = Number(field[1]);
                    }else if (field[0] === 'shopid'){
                        shopId = field[1];
                    }
                }  
            }
            ...
            let shopEl = shopList.find(item => item.id === shopId && item.onboard === true);
            if (!shopEl){
                res.status(200).json({no_authorization: 1});
                return;
            }
            ...
            const mealData = {...postedData, profileimage, created: new Date().toISOString()};
            const { id } = await db.collection('restaurants').doc(shopId).collection('mealmenu').add(mealData);
            res.status(200).json({ id, ...mealData } as MealDataType);
        }else{
