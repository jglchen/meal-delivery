...
import {UserJwtPayload, UserRecord, MealOrderType, NoAuthorization} from '@/lib/types';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<MealOrderType[] | NoAuthorization>) {
    try {         
        ...
        const {shopId, userId: userid, page} = req.query;
        ...
        const result: MealOrderType[] = [];
        const snapshot = await db.collection('restaurants').doc(shopId as string).collection('mealorders').where('userId', '==', userid).orderBy('created', 'desc').limit(limitNum).get();
        snapshot.forEach(doc => {
            result.push({id: doc.id, ...doc.data()} as MealOrderType);
        });
