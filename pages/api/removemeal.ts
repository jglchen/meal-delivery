...
import {UserJwtPayload, UserRecord, MealDelete, NoAuthorization, ProcessResult} from '@/lib/types';
const APP_SECRET = process.env.NEXT_PUBLIC_JWT_APP_SECRET as string;
import type { NextApiRequest, NextApiResponse } from 'next';

export default async function handler(req: NextApiRequest, res: NextApiResponse<MealDelete | NoAuthorization | ProcessResult>) {
    try {
        if (req.method === 'DELETE'){
            ...
            const resp = await db.collection('restaurants').doc(shopId).collection('mealmenu').doc(id).delete();
            res.status(200).json({meal_delete: 1});   
        }else{
