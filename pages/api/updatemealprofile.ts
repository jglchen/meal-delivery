...
export default async function handler(req: NextApiRequest, res: NextApiResponse< MealProfile | NoAuthorization | ProcessResult>) {
    try {
        if (req.method === 'POST'){
            //Check Authorization
            ...
            const resp = await db.collection('restaurants').doc(shopId).collection('mealmenu').doc(id).update({profileimage});
            res.status(200).json({shopId, id, profileimage});   
        }else{
