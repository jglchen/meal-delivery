...
import {UserJwtPayload, UserRecord, ShopUpdate, NoAuthorization, ProcessResult} from '@/lib/types';
const APP_SECRET = process.env.NEXT_PUBLIC_JWT_APP_SECRET as string;
import type { NextApiRequest, NextApiResponse } from 'next';

export default async function handler(req: NextApiRequest, res: NextApiResponse<ShopUpdate | NoAuthorization | ProcessResult>) {
    try {
        if (req.method === 'POST'){
            //Check Authorization
            ...
            const resp = await db.collection('restaurants').doc(shopId).update(others);
            res.status(200).json({shop_update: 1});   
        }else{
